...
def hash_identify(hash):
    ...
    if len(jerar)==0:
        ...
        jerar.sort()
        output += "\nPossible Hashs:"
        output += "\n[+] "+str(algorithms[jerar[0]])
        output += "\n[+] "+str(algorithms[jerar[1]])
        # print("\nPossible Hashs:")
        # print("[+] "+str(algorithms[jerar[0]]))
        # print("[+] "+str(algorithms[jerar[1]]))
        # possible.append(str(algorithms[jerar[0]]))
        # possible.append(str(algorithms[jerar[1]]))
        # print("\nLeast Possible Hashs:")
        output += "\n\nLeast Possible Hashs:"
        for a in range(int(len(jerar))-2):
            output += "\n[+] "+str(algorithms[jerar[a+2]])
            # least_possible.append(algorithms[jerar[a+2]])
            # print("[+] "+str(algorithms[jerar[a+2]]))
    else:
        jerar.sort()
        output += "\nPossible Hashs:"
        # print("\nPossible Hashs:")
        for a in range(len(jerar)):
            output += "\n[+] "+str(algorithms[jerar[a]])
            # possible.append(str(algorithms[jerar[a]]))
            # print("[+] "+str(algorithms[jerar[a]]))
